...
from PyQt5.QtCore import Qt, QTimer, QRectF, QPointF, QFileSystemWatcher
...
from utils.settings_manager import SettingsManager, apply_theme, HardwareDetectThread, grade_gpu_memory, grade_system_memory, load_hardware_cache, save_hardware_cache, resolve_theme, load_workspace, save_workspace_section, clear_workspace_window, apply_dwm_dark_mode, build_arch_cache, resolve_app_path
...
class BannerImportWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("旗帜训练导入器 v0.5 beta1")
        ...
        self.setStyleSheet(f"""
            ...
            QTabWidget::pane {{ font-size: {base_fs}px; }}
            QTabBar::tab {{ font-size: {btn_fs}px; padding: {max(int(4*self._scale),3)}px {max(int(10*self._scale),8)}px; }}
        ...
        icon_path = os.path.join("images", "icons", "importer.png")
    ...
    def _menu_about(self):
        QMessageBox.about(self, "关于", "旗帜训练导入器 v0.5 beta1\n\n用于旗帜序列的编辑、标记与训练数据导入。")
    ...
    def _do_auto_save(self):
        ...
        try:
            ...
            try:
                formats = SettingsManager().get("importer_auto_save_formats", ["mbtl"])
            except Exception:
                formats = ["mbtl"]
            ...
            if "all" in formats:
                formats = ["mbtl", "mbtlx"]
            sm = SettingsManager()
            base_dir = resolve_app_path(sm.get("auto_save_loader_path", "training_datas/auto_save/loader"))
            ...
            if "mbtlx" in formats:
                save_path = os.path.join(save_dir, "banner_auto.mbtlx")
                write_mbtl(save_path, self.banners)
                saved_names.append(os.path.basename(save_path))
            # 清理超限的自动保存文件夹
    ...
    def apply_ultra_wide_layout(self):
        ...
        h = self.height()
        if w > 0:
            self._main_splitter.setSizes([w // 4, 3 * w // 4])
        if h > 0:
            self._right_splitter.setSizes([h // 2, h // 2])
    ...
    def _create_preview_area(self):
        ...
        self.preview_group.setStyleSheet("QGroupBox { padding-top: 16px; }")
    ...
    def _create_right_panel(self):
        ...
        self.right_group.setStyleSheet("QGroupBox { padding-top: 16px; }")
    ...
    def _create_graphic_image_area(self):
        ...
        self.graphic_image_group.setStyleSheet("QGroupBox { padding-top: 16px; }")
    ...
    def _create_graphic_banner_preview(self):
        ...
        self.graphic_banner_group.setStyleSheet("QGroupBox { padding-top: 16px; }")
    ...
    def _create_graphic_save_panel(self):
        ...
        self.graphic_save_group.setStyleSheet("QGroupBox { padding-top: 16px; }")
    ...
    def _create_compact_panel(self):
        ...
        self._seq_buttons = [
            self._compact_del_btn, self._compact_edit_btn,
    ...
    def _create_expanded_panel(self):
        ...
        self._seq_buttons.extend([
            ...
            self._exp_del_btn, self._exp_edit_btn, self._exp_import_btn,
    ...
    def _on_tab_changed(self, index):
        ...
        if index == 0:
            ...
            # 进入 Tab2：从工作区文件恢复或默认五五开
            QTimer.singleShot(0, self._graphic_restore_or_default)
    ...
    def _graphic_add_pattern(self):
        selected = self._graphic_pattern_tree.selectedItems()
        if not selected:
            QMessageBox.warning(self, "提示", "请先选择图案")
            return
        if self._graphic_pattern_color is None:
            QMessageBox.warning(self, "提示", "请先选择图案颜色")
            return
        if (len(self._graphic_current_banner) - 1) // 2 >= 16:
            QMessageBox.warning(self, "提示", "图案层数已达上限(16层)")
            return
    ...
    def _graphic_mark_to_image(self):
        if self._graphic_current_img_idx < 0:
            QMessageBox.warning(self, "提示", "请先导入并选择图片")
            return
        if len(self._graphic_current_banner) < 1:
            QMessageBox.warning(self, "提示", "请先编辑旗帜数据")
            return
    ...
    def _graphic_import_images(self):
        ...
        if skipped:
            QMessageBox.warning(self, "部分文件被跳过",
    ...
    def _graphic_export_marks(self):
        if not self._graphic_marks:
            QMessageBox.warning(self, "提示", "没有可导出的标记")
            return
        ...
        if not save_path.endswith('.mbtlx'):
            save_path += '.mbtlx'
        try:
            # 新格式：ZIP 打包，内含 images/ + marks.json
            items = []
            img_counter = 0
            with zipfile.ZipFile(save_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                for img_path, banner_data in self._graphic_marks:
            ...
            QMessageBox.information(self, "导出成功", f"已导出 {len(items)} 条标记（含图片）")
        except Exception as e:
    ...
    def edit_saved_banner(self):
        ...
        rows = [active.row(item) for item in active.selectedItems()]
        if not rows:
            QMessageBox.warning(self, "警告", "请先选择要编辑的旗帜")
            return
        if len(rows) > 1:
            QMessageBox.warning(self, "警告", "只能编辑单个旗帜，请选择一个旗帜")
            return
        idx = rows[0]
        if idx >= len(self.banners):
            return
    ...
    def _delete_selected(self):
        ...
        if not rows:
            QMessageBox.warning(self, "警告", "请先选择要删除的旗帜")
            return
        if len(rows) == 1:
            msg = f"确定要删除旗帜 {rows[0]+1} 吗？"
        else:
            msg = f"确定要删除选中的 {len(rows)} 个旗帜吗？"
    ...
    def _delete_all(self):
        if not self.banners:
            return
        if QMessageBox.question(self, "确认删除", f"确定要删除全部 {len(self.banners)} 个旗帜吗？") == QMessageBox.Yes:
            self.banners.clear()
            self._after_delete()
    ...
    def open_delete_dialog(self):
        if not self.banners:
            QMessageBox.warning(self, "警告", "没有可删除的旗帜")
            return
        active = self._get_active_list()
        rows = [active.row(item) for item in active.selectedItems()]
        ...
        dlg.setFixedSize(int(320 * self._scale), int(280 * self._scale))
        layout = QVBoxLayout(dlg)
        ...
        if rows:
            ...
            def delete_selected():
                if len(rows) == 1:
                    ...
                    msg = f"确定要删除选中的 {len(rows)} 个旗帜吗？"
                if QMessageBox.question(dlg, "确认删除", msg) == QMessageBox.Yes:
                    for idx in reversed(rows):
                        if idx < len(self.banners):
                            del self.banners[idx]
                    self._after_delete()
        ...
        def delete_all():
            if QMessageBox.question(dlg, "确认删除", f"确定要删除全部 {len(self.banners)} 个旗帜吗？") == QMessageBox.Yes:
                self.banners.clear()
                self._after_delete()
                dlg.accept()
                QMessageBox.information(self, "删除成功", "已删除全部旗帜")
        ...
        def delete_range():
            ...
            if not text:
                QMessageBox.critical(dlg, "错误", "请输入要删除的范围，例如：1~12,20~31")
                return
            ...
            for part in parts:
                ...
                if "~" in part:
                    ...
                    try:
                        ...
                        QMessageBox.critical(dlg, "错误", f"无法解析范围：{part}")
                        return
                else:
                    try:
                        s = e = int(part)
                    except ValueError:
                        QMessageBox.critical(dlg, "错误", f"无法解析：{part}")
                        return
                if s < 1 or e > len(self.banners) or s > e:
                    QMessageBox.critical(dlg, "错误", f"范围无效：{part}（有效范围：1~{len(self.banners)}）")
                    return
                for i in range(s, e + 1):
                    indices.add(i - 1)
            cnt = len(indices)
            if cnt == 0:
                QMessageBox.critical(dlg, "错误", "未指定任何有效范围")
                return
            if QMessageBox.question(dlg, "确认删除", f"确定要删除 {text} 共{cnt}个旗帜吗？") == QMessageBox.Yes:
                for idx in sorted(indices, reverse=True):
                    if idx < len(self.banners):
                        del self.banners[idx]
                self._after_delete()
                dlg.accept()
                QMessageBox.information(self, "删除成功", f"已删除{cnt}个旗帜")
    ...
    def open_random_settings(self):
        ...
        dlg.setFixedSize(int(420 * self._scale), int(420 * self._scale))
        ...
        def do_corr_generate():
            ...
            if source_banners is None:
                QMessageBox.warning(self, "生成失败", "请先在旗帜列表中选中旗帜")
                return
            ...
            if ct == 0 and not vb and not vpc:
                QMessageBox.warning(self, "参数错误", "颜色纠偏至少需要选择一个变化维度")
                return
            ...
            if generated > 0:
                QMessageBox.information(self, "生成成功", f"已基于 {len(source_banners)} 个选中旗帜生成 {generated} 个{mode_name}旗帜")
            else:
                QMessageBox.warning(self, "生成失败", "未能生成任何旗帜，请检查参数设置")
            dlg.accept()
    ...
    def random_generate_banner(self):
        ...
        if generated_count > 0:
            if dedup_mode:
                QMessageBox.information(self, "生成成功", f"已生成 {generated_count} 个旗帜（无重复）")
            elif avoid_duplicate_banners:
                QMessageBox.information(self, "生成成功", f"已生成 {generated_count} 个旗帜（部分重复）")
            else:
                QMessageBox.information(self, "生成成功", f"已生成 {generated_count} 个旗帜")
    ...
    def _load_mbtl_file(self, filepath):
        try:
            ...
            QMessageBox.critical(self, "打开失败", f"文件格式无效:\n{str(e)}")
            return
        if not new_banners:
            QMessageBox.warning(self, "打开失败", "文件中没有旗帜数据")
            return
        if self.banners:
            dlg = QDialog(self)
        ...
        QMessageBox.information(self, "导入完成", f"已从 {os.path.basename(filepath)} 导入 {len(new_banners)} 个旗帜")

    def import_banners(self):
        sm = SettingsManager()
        _auto_path = resolve_app_path(sm.get("auto_save_loader_path", "training_datas/auto_save/loader"))
    ...
    def export_file(self):
        if not self.banners:
            QMessageBox.warning(self, "警告", "请先保存旗帜")
            return
        sm = SettingsManager()
        _auto_path = resolve_app_path(sm.get("auto_save_loader_path", "training_datas/auto_save/loader"))
        training_dir = os.path.dirname(os.path.dirname(_auto_path))
        ...
        QMessageBox.information(self, "导出成功", f"文件已导出至 {save_path}")
    ...
    def export_to_trainer(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "导出失败", f"写入信号文件失败: {e}")
            return
    ...
    def closeEvent(self, event):
        ...
        msg = "旗帜训练工具即将关闭，导入器与训练器将同时退出。\n\n"
        msg += "• 导入器中有未保存的序列数据，关闭后数据将丢失。\n"
        msg += "\n确定要退出吗？"
        self._launch_exit_confirmation("导入器", msg, False, has_data)
        event.ignore()
    ...
    def _do_exit(self):
        ...
        if save_requested and has_data:
            ...
            if "all" in save_fmts:
                save_fmts = ["mbtl", "mbtlx"]
            sm = SettingsManager()
            manual_base = resolve_app_path(sm.get("manual_save_loader_path", "training_datas/manual_save/loader"))
            manual_dir = os.path.join(manual_base, ts)
            os.makedirs(manual_dir, exist_ok=True)
            # 导入器无模型，仅保存旗帜数据 (.mbtl / .mbtlx)
            try:
                if "mbtl" in save_fmts:
                    write_mbtl(os.path.join(manual_dir, "banner.mbtl"), self.banners)
                if "mbtlx" in save_fmts:
                    write_mbtl(os.path.join(manual_dir, "banner.mbtlx"), self.banners)
            except Exception:
                pass
        self._remove_closing_signal()
        self._write_quit_signal()
...
if __name__ == "__main__":
    ...
    if "--debug" not in sys.argv:
        try:
            ...
            sys.stdout = open(os.path.join(_log_dir, "importer_stdout.log"),
    ...
    app.setApplicationName("旗帜训练工具 v0.5 beta1")
    app.setDesktopFileName("旗帜训练工具 v0.5 beta1")
    ...
    app.setStyleSheet(f"""
        ...
        QPushButton:disabled {{
            background-color: {'#2d2d30' if is_dark else '#f5f5f5'};
            color: {'#777777' if is_dark else '#aaaaaa'};
            border-color: {_g_input_border};
        }}
